...
**Phase 2K is under latency validation.** A bounded non-daemon controller owns one persistent child, loads the configured model once per session, preserves index-delimited segments, queues real `synthIndexReached` callbacks after segment playback, and isolates character-mode text. It keeps one active request plus one replaceable pending request, rejects stale PCM/index/completion events, and uses bounded worker cancellation and teardown. Phase 2K adds a silent worker warm-up and reuses one synthesis configuration per session. Language metadata is tolerated without model switching; unsupported prosody, break, and phoneme commands remain rejected. Piper, ONNX Runtime, models, WAV files, and the development environment remain outside the add-on. Warm bridge measurements and Phase 2J portable functional validation are recorded; physical audible-onset latency remains under investigation.
