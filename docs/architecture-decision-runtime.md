...
Phase 2J adds a non-daemon background controller around repeated one-shot children. Automated evidence shows one replaceable pending slot, generation/cancellation-token race protection, stale PCM and queued-completion rejection, background `WavePlayer` drain, and bounded shutdown. Controlled portable discovery and selection succeeded, but normal speech failed before controller submission because pinned `SpeechManager` appends an index item that the deliberately index-free prototype rejects. This leaves portable responsiveness and cancellation unmeasured; repeated cold loading, hard process termination, protocol design, packaging, and full sequence compatibility also remain unresolved, so the status remains Proposed.
